...
namespace SimpleOffenceSuppression
{
	namespace
	{
		// Shared gate: only suppress offence when the actor is a true neutral/civilian,
		// not a temporarily-calm enemy (toll bandit, Arvel, quest "friendly" hostiles).
		[[nodiscard]] bool ShouldSuppress(RE::Actor* a_subject, RE::Actor* a_player)
		{
			...
			if (Exclusions::GetSingleton()->IsExcluded(a_subject)) {
				return false;
			}
			return true;
		}
	...
	RE::FIGHT_REACTION IgnoreFriendlyFire::func(RE::Actor* a_subject, RE::Actor* a_player)
	{
		...
		if (ShouldSuppress(a_subject, a_player)) {
	...
	{
		if (ShouldSuppress(a_subject, a_player) && a_player->IsInCombat()) {
			return Settings::GetSingleton()->newReaction;
		}
		return RE::FIGHT_REACTION::kNeutral;
	}
	...
	void InstallOnDataLoad()
	{
		...
		// Resolve faction/keyword lists now that the load order is live
		Exclusions::GetSingleton()->CacheForms();
		...
		logger::info("Hostile Safeguard exclusions active");
	}
}
